...
/// Constructor.
HomographyModel::HomographyModel(const std::vector<Match> &m,
                                 const int width1, const int height1,
                                 const int width2, const int height2,
                                 bool symError)
: ModelEstimator(Match::toMat(m), symError), N1_(3, 3), N2_(3, 3),
  _areaLeft(height1 * width1), _areaRight(height2 * width2) {
    double stat[12];
    ...
    N1_(0, 2) = -stat[2] * dNorm;
    N1_(1, 2) = -stat[5] * dNorm;
    N2_(0, 2) = -stat[8] * dNorm;
    N2_(1, 2) = -stat[11] * dNorm;
}
...
/// For this test, it is assumed that det(H)>0.
/// \param[in] H The homography matrix.
/// \param[in] testData The correspondence to compute the error on.
/// \param[out] side In which image is the error measured?
/// \return The square reprojection error.
double HomographyModel::Error(const Model &H, Mat testData, int *side) const {
    double err = std::numeric_limits<double>::infinity();
    if (side) *side = 1;
    libNumerics::vector<double> x(3);
    // Transfer error in image 2
    ...
    if (x(2) > 0) {
        x /= x(2);
        err = sqr(testData(2,0)-x(0)) + sqr(testData(3,0) - x(1));
    }
    // Transfer error in image 1
    if (symError) { // ... but only if requested
        x(0) = testData(2, 0);
        ...
        if (x(2) > 0) {
            x /= x(2);
            double err1 = sqr(testData(0,0)-x(0)) + sqr(testData(1,0)-x(1));
            if (err1 > err) { // Keep worse error
                err = err1;
                if (side) *side = 0;
            }
...
double HomographyModel::pSigma(const double sigma, const bool leftSide) const {
    double area;
    if (leftSide) area = _areaLeft;
    else area = _areaRight;
