...
template<typename T>
matrix<T> cat(const matrix<T> &, const matrix<T> &);

template<typename T>
matrix<T> stack(const matrix<T> &, const matrix<T> &);
...
public:
    ...
    /// Product by scalar.
    friend matrix<T> operator*(T a, const matrix<T> &m) { return m*a; }
...
public:
    ...
    friend vector<T> operator*(T a, const vector<T> &v) { return v*a; }
