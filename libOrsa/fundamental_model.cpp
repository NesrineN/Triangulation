...
* 
* Copyright (c) 2011-2021 Pascal Monasse
...
/// Constructor.
FundamentalModel::FundamentalModel(const std::vector<Match> &m,
                                   const int width1, const int height1,
                                   const int width2, const int height2,
                                   bool symError)
: ModelEstimator(Match::toMat(m), symError), N1_(3, 3), N2_(3, 3),
  _areaLeft(width1 * height1), _areaRight(width2 * height2),
  _diameterLeft(std::sqrt(std::pow(width1, 2) + std::pow(height1, 2))),
  _diameterRight(std::sqrt(std::pow(width2, 2) + std::pow(height2, 2))) {
...
/// \param xa, xb, ya, yb The matches coordinates.
/// \param side In which image is the error measured?
/// \return The square reprojection error.
double FundamentalModel::Error(const Model &F,
...
double FundamentalModel::Error(const Model &F, int index, int *side) const {
    ...
    return Error(F, xa, xb, ya, yb, side);
}

/// \param F The fundamental matrix.
/// \param testData The point correspondence.
/// \param side In which image is the error measured?
/// \return The square reprojection error.
double FundamentalModel::Error(const Model &F, Mat testData, int *side) const {
    double xa = testData(0, 0), ya = testData(1, 0);
    double xb = testData(2, 0), yb = testData(3, 0);
    return Error(F, xa, xb, ya, yb, side);
}
...
double FundamentalModel::pSigma(const double sigma, const bool leftSide) const {
    double area, diameter;
    if (leftSide) {
        area = _areaLeft;
        diameter = _diameterLeft;
    } else {
        area = _areaRight;
        diameter = _diameterRight;
    }
    return 2 * sigma * diameter / (double) area;
}
