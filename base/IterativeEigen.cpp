...
cv::Point3d triangulate_Iterative_Eigen(const cv::Vec3d& U, const cv::Vec3d& U_prime, const cv::Mat& P, const cv::Mat& P_prime) {
    double u = U[0], v = U[1];
    ...
    for (int i = 0; i < 10; i++) {
        ...
        cv::Mat W, U_svd, VT;
        cv::SVD::compute(A, W, U_svd, VT);
        solution_2 = VT.row(3); // 1x4 solution
